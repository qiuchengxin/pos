...
public class SellController_special {
    ...
    public static void sellController(String sell_userid,String buy_userid,String groupid){
        ...
        //查询是否表中有销售者的数据
        MarketService_special.selectMarketByUserid(sell_userid,groupid);
        String market_userid = MarketService_special.userid;
        String market_buy_userid = MarketService_special.buy_userid;
        System.out.println("marketuserid : "+market_userid);

        if (market_userid != null){

            MarketService_special.selectMarketByUserid(sell_userid,groupid);
            String good = MarketService_special.good;
            String money = MarketService_special.money;
            //查询销售者的背包
            BackpackService.searchBackPack(sell_userid,groupid);
            String sell_data_equipment = BackpackService.special;
          ...
          if (market_buy_userid == null){
              ...
              if (buy_user_money_String == null){
                  ...
                  //获取交易后的销售者数据
                  String end = Sell.end;
                  BackpackService.updateBackPack(sell_userid,"special",end,groupid);
                  ...
                  String buy_data_equipment = BackpackService.special;
                  if (resultUserid == null){
                      ...
                      if (buy_data_equipment == null){
                          //购买者没有秘籍
                          BackpackService.updateBackPack(buy_userid,"special",good,groupid);
                          //update购买者id
                          MarketService_special.updateBuy_userid(sell_userid,buy_userid,groupid);
                      }else {
                          //购买者包里已经有秘籍了
                          String last_equipment = buy_data_equipment + "," + good;
                          BackpackService.updateBackPack(buy_userid,"special",last_equipment,groupid);
                          //update购买者id
                          MarketService_special.updateBuy_userid(sell_userid,buy_userid,groupid);
                      }
                  }else {
